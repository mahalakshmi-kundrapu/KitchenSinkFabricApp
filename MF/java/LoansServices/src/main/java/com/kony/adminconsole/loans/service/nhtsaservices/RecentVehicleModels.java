...
public class RecentVehicleModels implements JavaService2{

	/**
	 * Logger logger used to log the debug/info/error statements.
	 */
	private static final Logger LOGGER = Logger.getLogger(RecentVehicleModels.class);
	/**
		...
		try{
			...
			while (true) {
				...
				for(int i=0; i<vehicleTypes.length();i++){
					...
					Future<JSONObject> future = executor.submit(new RecentVehicleModelsCallable(object , dcRequest));
					list.add(future);
				}
	...
	private JSONArray hitVehicleMakeGetService(HashMap<String, String> inputParams, DataControllerRequest dcRequest, int batch) {
		...
		{
			filter = "?$filter=MakeName%20eq%20'"+inputParams.get("MakeName")+"'&$orderby=id&$top=" + BATCHSIZE + "&$skip=" + batch;
		}else {
