...
public class CreateLoanAddAppIdInSessionPostProcessor implements ObjectServicePostProcessor{

	private static final Logger log = Logger
			.getLogger(CreateLoanAddAppIdInSessionPostProcessor.class);
	/**
	 * @author kh2267
	 */
	@Override
	public void execute(FabricRequestManager fabricRequestManager, FabricResponseManager fabricResponseManager) throws Exception {
		...
		try {
		    ...
		    if(responseAsPayloadJSON.has("id") && responseAsPayloadJSON.getString("id")!=null) {
		    		String ApplicationId = responseAsPayloadJSON.getString("id");
