...
public class PeriodLimitEndUserUpdate implements JavaService2 {
	private static final Logger LOG = Logger.getLogger(PeriodLimitEndUserUpdate.class);
		...
		try {
			...
			try {
				...
				for (int i = 0; i < periodsLimitArray.length(); i++) {
					...
					deleteFlag = ((JSONObject) periodsLimitArray.get(i)).optString("deleteFlag").trim();
					...
					if (periodicLimitId == null || periodicLimitId.isEmpty() || periodicLimitId.trim().length() == 0) {
						...
						JSONObject updatePeriodicLimitResponseJSON = updatePeriodicLimit(periodicLimitId,
								transactionLimitId, periodId, periodLimit, authToken, systemUser, requestInstance);
						if (updatePeriodicLimitResponseJSON != null
								&& updatePeriodicLimitResponseJSON.getInt(FabricConstants.OPSTATUS) != 0) {
				...
				AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS, EventEnum.UPDATE,
						ActivityStatusEnum.FAILED, "Periodic Limit update failed. errmsg: " + e.getMessage());
	...
	private JSONObject createPeriodicLimit(String periodicLimitId, String transactionLimitId, String periodId,
		...
		AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS, EventEnum.CREATE,
				ActivityStatusEnum.SUCCESSFUL, "Periodic Limit Create success. periodicLimitId: " + periodicLimitId);
		return createResponseJSON;
	}
	...
	private JSONObject updatePeriodicLimit(String periodicLimitId, String transactionLimitId, String periodId,
		...
		if (getOpStatusCode != 0) {
			// rollback logic
			AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS, EventEnum.UPDATE,
					ActivityStatusEnum.FAILED, "Periodic Limit update failed. periodicLimitId: " + periodicLimitId);
			return updateResponseJSON;
		}
		AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS, EventEnum.UPDATE,
				ActivityStatusEnum.SUCCESSFUL, "Periodic Limit update success. periodicLimitId: " + periodicLimitId);
		return updateResponseJSON;
	}
	...
	private JSONObject deletePeriodicLimit(String periodicLimitId, String authToken, String systemUser,
		...
		postParametersMap.clear();
		postParametersMap.put("id", periodicLimitId);
		String updatePeriodicLimitResponse = Executor.invokeService(ServiceURLEnum.PERIODICLIMIT_DELETE,
				postParametersMap, null, requestInstance);
		...
		if (getOpStatusCode != 0) {
			...
			AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS, EventEnum.DELETE,
					ActivityStatusEnum.FAILED, "Periodic Limit delete failed. periodicLimitId: " + periodicLimitId);
			return updateResponseJSON;
		}
		AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS, EventEnum.DELETE,
				ActivityStatusEnum.SUCCESSFUL, "Periodic Limit delete success. periodicLimitId: " + periodicLimitId);
		return updateResponseJSON;
	}
}