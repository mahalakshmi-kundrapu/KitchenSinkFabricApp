...
public class TransactionFeeUserGroupUpdate implements JavaService2 {
	...
	public Object invoke(String methodID, Object[] inputArray, DataControllerRequest requestInstance,
		...
		if (requestInstance.getParameter("serviceId") != null) {
			...
			errorCodeParam = new Param("errorCode", "5011", FabricConstants.STRING);
			errorMessageParam = new Param("errorMessage", "SERVICE_ID_EMPTY", FabricConstants.STRING);
			processedResult.addParam(errorCodeParam);
			processedResult.addParam(errorMessageParam);
			return processedResult;
		}
		if (requestInstance.getParameter("groupId") != null) {
			groupId = requestInstance.getParameter("groupId");
		} else {
			errorCodeParam = new Param("errorCode", "5011", FabricConstants.STRING);
			errorMessageParam = new Param("errorMessage", "GROUP_ID_EMPTY", FabricConstants.STRING);
			processedResult.addParam(errorCodeParam);
			processedResult.addParam(errorMessageParam);
			return processedResult;
		}
		if (requestInstance.getParameter("systemUserId") != null) {
			...
			errorCodeParam = new Param("errorCode", "5011", FabricConstants.STRING);
			errorMessageParam = new Param("errorMessage", "SYSTEM_USER_EMPTY", FabricConstants.STRING);
			processedResult.addParam(errorCodeParam);
			processedResult.addParam(errorMessageParam);
			return processedResult;
		}
		...
		if (requestInstance.getParameter("transactionFeeId") != null) {
			...
			errorCodeParam = new Param("errorCode", "5014", FabricConstants.STRING);
			errorMessageParam = new Param("errorMessage", "TRANSACTION_FEE_ID_EMPTY", FabricConstants.STRING);
			processedResult.addParam(errorCodeParam);
			processedResult.addParam(errorMessageParam);
			return processedResult;
		}
		...
		try {
			...
			if (prevPL != null && prevPL.getInt(FabricConstants.OPSTATUS) != 0) {
				ErrorCodeEnum.ERR_20424.setErrorCode(processedResult);
			}
			...
			for (int i = 0; i < transactionFeeSlabsArray.length(); i++) {
				...
				if (("Y").equalsIgnoreCase(deleteFlag)) {
					...
					if (deletePeriodicLimitResponseJSON != null && deletePeriodicLimitResponseJSON.getInt(FabricConstants.OPSTATUS) != 0) {
						ErrorCodeEnum.ERR_20808.setErrorCode(processedResult);
					}
					continue;
				}
				...
				if (slabId == null || slabId.isEmpty() || slabId.trim().length() == 0) {
					...
					if (createPeriodicLimitResponseJSON != null && createPeriodicLimitResponseJSON.getInt(FabricConstants.OPSTATUS) != 0) {
						ErrorCodeEnum.ERR_20805.setErrorCode(processedResult);
					}
					continue;
				} else {
					// update
					...
					if (updateTransferFeeSlabResponseJSON != null && updateTransferFeeSlabResponseJSON.getInt(FabricConstants.OPSTATUS) != 0) {
						ErrorCodeEnum.ERR_20807.setErrorCode(processedResult);
					}
					continue;
				}
			}
			...
			AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS,
					...
					"Transaction fee create failed. errmsg: " + e.getMessage());
			result.addParam(statusParam);
		}
		AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS,
				EventEnum.CREATE, ActivityStatusEnum.SUCCESSFUL,
				"Transaction fee create successful.");
		return result;
	...
	private JSONObject deleteTransferFeeSlab(String slabId, String authToken, String systemUser,
		...
		if (getOpStatusCode != 0) {
			...
			AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS,
					...
					"transactoin fee slab delete failed. TransactionSlabId: " + slabId);
			return updateResponseJSON;
		}
		AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS,
				EventEnum.DELETE, ActivityStatusEnum.SUCCESSFUL,
				"transactoin fee slab delete success.TransactionslabId: " + slabId);
		return updateResponseJSON;
	}
	...
	private JSONObject fetchGroupEntitlement(String groupId, String serviceId, String authToken,
		...
		String viewResponse = Executor.invokeService(
				ServiceURLEnum.TRANSACTIONFEEGROUP_VIEW_READ, postParametersMap,
	...
	private JSONObject createTransactionFeeSlab(String serviceId, String transactionFeeId,
		...
		for (int i = 0; i < transactionFeeSlabsArray.length(); i++) {
			...
			AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS,
					EventEnum.CREATE, ActivityStatusEnum.SUCCESSFUL,
					"transactoin fee slab create success.transactionFeeSlabId: " + transactionFeeSlabId);
		}
		return createResponseJSON;
	}
	...
	private JSONObject updateTransferFeeSlab(String slabId, String transactionFeeId, String minLimit, String maxLimit,
		...
		AuditHandler.auditAdminActivity(requestInstance, ModuleNameEnum.PERIODICLIMITS,
				...
				"transactoin fee slab update success.transactionFeeSlabId: " + slabId);
		return updateResponseJSON;
	}
}